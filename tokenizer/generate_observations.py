...
class ObservationGenerator:
    ...
    def __init__(self, maps, data, cfg,n_actions = 1):
    ...
    def generate_observations_kuioma(self, start_range, end_range,n_actions=5):
        self.n_actions = n_actions
        def find_last_non_zero_index(actions):
            for i in reversed(range(len(actions))):
                if actions[i] != 0:
        ...
        def get_all_maps(grid,paths):
            #start with id:100
            path_len = len(paths[0])
            all_maps = []
            for t in range(path_len):
                temp_grid = copy.deepcopy(grid)
                for id,agents_loc in enumerate(paths):
                    id = 100+id
                    temp_grid[agents_loc[t][0]][agents_loc[t][1]] = id
                all_maps.append(temp_grid)
            return all_maps
        ...
        for instance_id in range(start_range, end_range):
            # inputs {map:Int[Array,"n_steps,gird_shape"],actions:Int[Array,"n_agents,action_step"],finish:Bool[Array,"action_step"}
            # temp_inputs = {"maps":[],
            #                "actions":[],
            #                "agents_input":[[] for i in range(self.data[instance_id]["env_grid_search"]["num_agents"])],
            #                "finish":[]}
            if self.data[instance_id]["metrics"].get("CSR", 1) < 1:
                continue
            if map_name != self.data[instance_id]["env_grid_search"]["map_name"]:
            ...
            all_maps = get_all_maps(grid,paths)
            # temp_inputs["maps"] = all_maps
            # temp_inputs["actions"] = self.data[instance_id]["metrics"]["made_actions"][:][:]
            ...
            for agent_id in range(len(paths)):
                ...
                for t in range(len(paths[agent_id])):
                    agents = self.generate_agent_info_real_action(
                        agent_id, t, proximity_lists[t][agent_id], paths, goal_positions
                    )
                    cost2go_obs = self.generate_cost2go_obs(
                        self.cost2go_data[tuple(paths[agent_id][-1])],
                    ...
                    # temp_inputs["agents_input"][agent_id].append(                        
                    #     np.array(
                    #         self.encoder.encode(agents, cost2go_obs), dtype=np.int8
                    #     )
                    # )
                    ...
                    gt_actions = np.zeros(self.n_actions,dtype=np.int8)
                    if t+self.n_actions-1 < len(paths[0]):
                        gt_actions = self.data[instance_id]["metrics"]["made_actions"][agent_id][t:t+self.n_actions]
                    else:
                        gt_actions[:len(paths[0])-t] = self.data[instance_id]["metrics"]["made_actions"][agent_id][t:]
                    self.gt_actions.append(
                        gt_actions
                    )
                    # self.gt_actions.append(
                    ...
                    if t > goal_t:
                        self.gt_actions[-1] = 5  # 5 is a wait in goal
            # inputs_new.append(temp_inputs)
        return self.inputs, self.gt_actions,inputs_new
    ...
    def get_agent_paths(self, actions_list, initial_positions):
        ...
        return all_paths
    def get_agent_info_real_action(self, agent_id, time, paths, goal_positions, central_position,random_change=True,random_change_prob=0.3):
        path = paths[agent_id]
        agent_position = tuple(path[min(time, len(path) - 1)])
        agent_next_position = tuple(path[min(time+1, len(path) - 1)])
        ...
        next_action = ""
        dy,dx = agent_next_position[0]-agent_position[0],agent_next_position[1]-agent_position[1]
        for m in self.moves[1:]:
            if dy == m[0] and dx == m[1]:
                next_action += "1"
            else:
                next_action += "0"
        p = np.random.rand()
        if p>1-random_change_prob:
            next_action = np.zeros(5,dtype=int)
            next_action[np.random.randint(0,5)] = 1
            next_action = ''.join(map(str,next_action[1:]))
    ...
    def generate_agent_info_real_action(
        self, agent_id, time, proximity_list, paths, goal_positions
    ):
        agents_info = []
        for id in proximity_list[: self.cfg.num_agents]:
            if id == agent_id:
                ...
                agent_info = self.get_agent_info_real_action(
                    id, time, paths, goal_positions, paths[agent_id][time]
                )
            agents_info.append(agent_info)
