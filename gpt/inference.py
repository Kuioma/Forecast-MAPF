...
class MAPFGPTInferenceConfig(AlgoBase, extra=Extra.forbid):
    ...
    repo_id: str = 'Kuioma00614/Forecast-MAPF'
    action_n = 1
    predict_next_action = False # if false greedy action in agent info else predict action in agent info
...
class MAPFGPTInference:
    ...
    def act(self, observations):
        ...
        tensor_obs = torch.tensor(
            [self.encoder.encode(input,id=index,predict_action=self.predict_next_action) for index,input in enumerate(inputs)],
            dtype=torch.long,
