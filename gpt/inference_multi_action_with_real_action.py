...
###
# 主要变化:原本的greedy action变为了预测的action
# 维护一个列表记录每个agent预测的动作

class MultiActionMAPFGPTInferenceConfig(AlgoBase, extra=Extra.forbid):
...
class MultiActionGPTInference:
    def __init__(self, cfg: MultiActionMAPFGPTInferenceConfig, net=None):
        self.cfg = cfg
    ...
    def generate_input(self, observations):
        ...
        predict_next_actions = ["" for _ in range(len(observations))]
        for agent_idx, obs in enumerate(observations):
            next_action = ""
            for m in [[-1, 0], [1, 0], [0, -1], [0, 1]]: #上下左右
                ...
                if (
                    ...
                    next_action += "0"
            predict_next_action = self.net.get_next_action(agent_idx)
            if predict_next_action is None:
                predict_next_action = next_action
            predict_next_actions[agent_idx] = predict_next_action
                ...
                if -self.cfg.agents_radius <= relative_xy[0] <= self.cfg.agents_radius \
                    ...
                    agents_info.append(
                        {   
                            ...
                            "predict_next_action": predict_next_actions[n],
                        }
                    )
            inputs.append(
                {
    ...
    def act(self, observations):
        ...
        tensor_obs = torch.tensor(
            [self.encoder.encode(input,index) for index,input in enumerate(inputs)],
            dtype=torch.long,
