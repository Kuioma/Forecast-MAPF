...
def split_list_into_n_parts(lst, n):
    ...
    Returns:
        list: 包含 n 个子列表的列表。
              如果 n > len(lst)，则会有些子列表为空。
              如果 n <= 0，会抛出 ValueError。
    """
    if not isinstance(lst, list):
        raise TypeError("输入 'lst' 必须是一个列表。")
...
def run_episode(env, algo):
    ...
    results_holder.results["map"] = env.grid.obstacles.astype(np.int8)
    # map_name 和 实际 map对不上
    total_map_name = env.grid_config.map_name.split("seed")
    results_holder.results["map_name"] = total_map_name[0][:-1]
    results_holder.results["seed"] = total_map_name[1][1:]
    while True:
...
def evaluation(evaluation_config, eval_dir=None,n=25):
    """
    ...
    environment_configs_split = split_list_into_n_parts(list(environment_configs),n)
    results = []
    for index,environment_configs in enumerate(environment_configs_split):
        for key, algo_cfg in evaluation_config['algorithms'].items():
            ...
            processed_episodes = []
            for episode in algo_results:
                processed_episode = {
                    'map_name':episode['metrics']['map_name'],
                    'map':json.dumps(episode['metrics']['map'].tolist()),
                    'actions':json.dumps(algo_results[0]['metrics']['made_actions']),
                    'init_pos':json.dumps(algo_results[0]['metrics']['init_positions'])
                }
                processed_episodes.append(processed_episode)
            df = pd.DataFrame(processed_episodes)
            if eval_dir:
                save_path = Path(eval_dir) / f'{key}_{index}.parquet'
    ...
    # run_views(results, evaluation_config, eval_dir=eval_dir)s
    return results
