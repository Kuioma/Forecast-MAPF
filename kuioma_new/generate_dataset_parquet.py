...
from evaluator1_1 import evaluation
...
def balance_and_filter_tensors(tensors, actions, known_hashes=None):
    # 筛选逻辑：
    # 1.根据obs的hash来筛选，重复的obs就不放入
    # 2.判断action为5，大于5表示后面都是保持不动，要丢掉
    new_tensors = []
    new_actions = []
    duplicates = 0
    ...
    if len(new_tensors) > 0:
        ...
        for action in new_actions:
            ...
            if type(action) == int:
                ...
                actions_made[action[0]] += 1
        i = len(new_tensors) - 1
        discarded = 0
        while i >= 0:
            if new_actions[i] is 5:
                if (actions_made[0] + actions_made[5]) > len(new_tensors) // 5:
                    ...
                    # new_actions[i] = 0
                    new_actions[i] = np.zeros(NUM_ACTIONS)
            i -= 1
...
def process_files(maze_files, random_files, output_file):
    ...
    for i in range(FILE_PER_CHUNK):
        ...
        gt_actions_col = pa.array(actions_chunk.tolist(),type=pa.list_(pa.int8()))
